...
| 删除/覆盖安全性 | 用户主动卸载、Base SMAPI 卸载、版本删除、普通 Mod 覆盖、嵌套 Mod 文件夹修复中的同名目标，以及 Collection 覆盖/回滚中的用户目录统一优先移入系统回收站；旧 WPF 实例删除也已改为先安全移除 junction，再将完整版本目录移入回收站；临时解压、缓存和事务暂存目录仍按生命周期物理清理 | `RecycleBinService`、`SettingsService`、`ModManager`、`FeaturePagesViewModels`、`InstancesPageViewModel`、`DownloadInstallService` |
