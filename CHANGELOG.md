...
- **任务入口与下载管理交互增强**
  ...
  - 新增右下角任务浮动按钮关闭入口，支持在设置页重新启用
  - 新增导航栏“任务”提示红点与锁存提醒（进入任务页后清除）
  - 新增启动页“Mod管理”快捷入口（位于版本选择与版本设置之间），SMAPI 实例可直接跳转到版本设置-模组管理页
- **Mod 分类兼容增强**

  - 新增对文件夹名前缀分类的识别：`[分类]` / `【分类】`
  - 新增前缀分类专用展示样式，和普通文件夹分类、自定义标签区分显示

### Changed

- **下载任务状态流转优化**

  - 重试未匹配 NXM 任务改为后台触发，避免动作命令被阻塞
  - 任务页状态同步逻辑优化，重试后可立即进入“可取消”交互
- **Nexus 回调与安装流程优化**

  - NXM 文件定位改为“下载前快照 + 新文件识别”，减少历史缓存误命中
  - 未匹配回调临时目录改为每任务独立目录，避免交叉污染
  - Nexus 2400（SMAPI）统一走实例安装流程，避免误入普通 Mod 安装流程
- **标签显示与筛选解析优化**

  - 标签显示前缀统一为三类：普通分类、前缀分类、自定义标签
  - 标签点击筛选兼容新旧显示格式（含 emoji 与文本前缀）

### Fixed

- 修复重试未匹配 NXM 任务后无法取消的问题
- 修复任务页重试按钮区域“取消”误绑定为返回页面的问题
- 修复 NXM 回调场景下下载完成后误取旧 ZIP（如 `Content Patcher.zip`）导致安装流程错误的问题
- 修复 Nexus Token 过期时重复触发高频日志与重复请求的问题
- 修复下载设置区任务管理器显示开关入口缺失与同步不稳定问题
...
- **Mod 标签管理可视化面板增强**
  ...
  - 新增 Tag chips 面板，支持新建、更改标签对 Mod 进行分类
  - 新增自定义标签右键菜单（重命名/删除）
  - 新增已选 Mod 的批量“添加标签/删除标签”操作，并并入主悬浮操作栏
  - 新增 Mod 行内标签点击筛选能力（点击标签即可按标签过滤），点击搜索后在搜索栏一键填充搜索条件
- **高级搜索能力扩展**

  - 新增高级搜索语法帮助入口（info 按钮）、一键清除（x 按钮）
  - 支持字段查询：`tag:`、`author:`、`description:`、`name:`、`uid:`、`folder:`
  - 支持状态关键词：`enabled:`、`disabled:`、`updatable:`
  - 支持 `OR` 组合与排除词（`-关键词`）
...
- **Tag 数据模型与持久化优化**
  ...
  - 自定义标签改为 ID 绑定，支持改名不影响已绑定关系
  - 标签配置新增排序持久化字段（文件夹顺序、自定义顺序）
  - Mod 行标签显示统一为“文件夹标签 + 自定义标签”并支持换行展示
- **Mod 管理页布局与交互优化**

  - 修复 Tag 管理区在窄宽度下溢出问题，筛选区与标签区改为可换行布局
  - 优化标签管理提示文案排版，提示信息独立成行并自动换行
  - 优化输入框在亮色模式下的可读性（前景/边框/背景对比增强）
  - 优化标签悬浮视觉反馈（悬浮加粗），提升可点击感知
- **构建产物管理优化**

  - Release/Debug 命名产物在构建时自动清理同配置历史版本，避免输出目录持续堆积
  - 输出目录打包体积显著降低，便于发布前校验与分发
- **前置依赖状态展示优化**

  - 必选依赖缺失/禁用时显示红色状态前缀（`[未安装]` / `[被禁用]`）
  - 可选依赖保留 `[可选]`标识；当缺失/禁用时按 `[未安装][可选]`或 `[被禁用][可选]`顺序显示，其中状态前缀为黄色
  - 使依赖更智能地进行换行

### Fixed

- 修复 Mod 被禁用的方式，使其不被 SMAPI 加载
- 修复文件夹标签右键出现空菜单的问题
- 修复标签选择清空后搜索栏未同步清空的问题
- 修复 Mod 管理页中标签与操作行在部分宽度下换行不稳定的问题
- 修复 Mod 行自定义标签与文件夹标签同名时，点击筛选误命中分组的问题
- 修复 Mods 页面因前置状态绑定模式导致的 XamlParseException
...
- **匿名用户统计（可关闭）**
  ...
  - 新增匿名活跃上报服务（本地匿名 ID + 每日一次上报）
  - 仅上报匿名哈希标识与基础运行信息，不采集用户名、路径等隐私数据
  - 支持通过环境变量 `SVL_DISABLE_ANON_TELEMETRY=1` 关闭统计
- **README 用户趋势展示**
...
- **游戏路径校验统一升级**
  ...
  - 路径选择弹窗、确认弹窗、下载页与实例选择器文案统一为“核心文件检测”逻辑
  - 不再写死依赖 `Stardew Valley.exe`，改为统一使用稳定文件校验
- **启动兼容逻辑增强**

  - 启动时智能解析可执行文件，兼容原版/SMAPI 启动文件改名或缺失场景
  - 遇到兼容回退时会给出明确提示，避免“原版/SMAPI 选择与实际启动不一致”无提示
- **版本设置页简化**

  - 移除“版本隔离”开关及该开关触发的隔离初始化流程
  - Base 实例默认不可隔离；SMAPI 安装流程保持默认写入 `versions` 目录

### Fixed

- 修复关于页“启动器更新”当前版本显示为三段号（如 `1.1.8`）的问题，现按四段号显示（如 `1.1.8.1`/`1.1.8.2`）
- 修复更新弹窗当前版本来源不一致导致的版本显示偏差
- 修复路径探测流程中仍旧按 `Stardew Valley.exe` 存在与否判断有效路径的问题
